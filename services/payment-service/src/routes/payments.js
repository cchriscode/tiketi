/**
 * Payments Router
 * Handles payment processing, webhook, and payment inquiry
 */
  ...
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (reservationResult.rows.length === 0) {
        throw new CustomError(404, ERROR_MESSAGES.RESERVATION_NOT_FOUND);
      }
      ...
      if (reservation.payment_status === PAYMENT_STATUS.COMPLETED) {
        throw new CustomError(400, 'Payment already completed for this reservation');
      }

      // Check if expired
      if (reservation.expires_at && new Date(reservation.expires_at) < new Date()) {
        throw new CustomError(400, ERROR_MESSAGES.RESERVATION_EXPIRED);
      }
